...
import sys,os
sys.path.append(os.pardir)    # 부모 디렉토리 파일을 가져오게끔 설정
...
def numerical_gradient(f,x):
    if x.ndim == 1:
        ...
        grad = np.zeros_like(x)  # x와 형상이 같고 그 원소가 모두 0인 배열 grad 생성
...
plt.plot( [-5, 5], [0,0], '--b')
plt.plot( [0,0], [-5, 5], '--b')
plt.plot(x_history[:,0], x_history[:,1], 'o')  # 값이 가장 낮은 장소인 원점에 가까워짐
...
4.4.2 신경망에서의 기울기
"""
from common.functions import softmax, cross_entropy_error    # common 파일 안에 있는 functions.py 에서 softmax, cross_entropy_error를 사용
from common.gradient import numerical_gradient         # common 파일 안에 있는 gradient.py 에서 를 numerical_gradient 사용
...
f = lambda w: net.loss(x, t)  # labmda 기법으로 함수 f 정의 => f(w) = net.loss(x,t)
dW = numerical_gradient(f, net.W)
print(dW)      # 2x3 차원 배열로 각 가중치 매개변수(2x3)마다 편미분 했을 때 손실함수가 얼마나 변하는지 의미
